...
### Fluxo 8 — Administração & Mediação  *(Épico 9 — US22–US28)*  · **WEB (desktop), NÃO mobile**
> Painel web em React, layout de backoffice (sidebar + tabela/cards). Consome os mesmos `tokens.css` (areia/oceano/turquesa) numa densidade maior, própria de desktop. Acesso `ROLE_ADMIN`.
28. **Login Admin** — acesso restrito; tela sóbria, selo oceano.
29. **Dashboard de métricas** — cards de KPI: GMV, receita de comissão, ticket médio, pedidos por status, taxa de conclusão, disputas abertas + tempo médio de resolução, prestadores verificados/ativos, clientes ativos, SOS acionados. Filtro por período/bairro. Gráficos simples.
30. **Fila de disputas** — tabela de `EM_DISPUTA` (pedido, partes, valor retido, idade). Abrir → **detalhe da disputa** com histórico, mídias e ações **Liberar (a favor do prestador)** / **Reembolsar (a favor do cliente)**.
31. **Moderação de prestadores** — lista filtrável por status; destaque para `INCONCLUSIVO`; ação **Verificar/Reprovar** com justificativa.
32. **Gestão de usuários** — busca por e-mail/nome; perfil; **Suspender/Reativar** (com confirmação, ação auditada).
33. **Reconciliação financeira** — tabela de `transactions` por status + fila de `outbox` em `FALHA` com botão **Reprocessar** (idempotente).
34. **Catálogo de categorias** — CRUD de categorias (criar/editar/desativar).
35. **Log de auditoria** — tabela append-only: admin, ação, entidade, quando.
36. **Central de notificações** — lista de alertas (SOS, disputa nova, verificação inconclusiva), não lidas em destaque; **alerta de SOS no topo, cor `--danger`**, com link para o pedido. Indicador de badge no header.
37. **Exportar relatório** — em dashboard/listagens, botão **Exportar** → CSV (listas) e PDF (resumo de métricas), respeitando filtros. Aviso "sem dados pessoais sensíveis".
...
**P0 (mostrar primeiro):** 1, 6, 7, 9, 10, 11, 15, 16, 19, 22 · **admin:** 29, 30, 36 (alerta SOS).
**P1:** 3, 4, 13, 14, 20, 24, 26 · **admin:** 28, 31, 33, 37.
**P2:** o restante (estados, status, confirmações, telas 32/34/35).
