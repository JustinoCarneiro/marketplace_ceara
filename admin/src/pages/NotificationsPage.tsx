...
function iconFor(tipo: string) {
  ...
  if (tipo === 'DISPUTA') return { bg: '#F7E3D6', el: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#A94C25" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10.3 3.9 2 18a2 2 0 001.7 3h16.6a2 2 0 001.7-3L13.7 3.9a2 2 0 00-3.4 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg> };
  if (tipo === 'VERIFICACAO') return { bg: '#FDF3D6', el: <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#8E6508" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3l7 3v6c0 4.5-3 7.5-7 9-4-1.5-7-4.5-7-9V6z"/><line x1="12" y1="9" x2="12" y2="12.5"/><line x1="12" y1="15" x2="12.01" y2="15"/></svg> };
      ...
      <div style={{ height: 64, flexShrink: 0, background: 'var(--surface)', borderBottom: '1px solid var(--line-soft)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 28px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <div style={{ position: 'relative', width: 44, height: 44, borderRadius: 12, background: '#F3ECDC', border: '1px solid #E6DDC9', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            {unreadCount > 0 && <span style={{ position: 'absolute', top: -5, right: -5, minWidth: 19, height: 19, padding: '0 5px', borderRadius: 100, background: '#10847D', color: '#fff', fontSize: 12, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid var(--surface)' }}>{unreadCount}</span>}
          </div>
        </div>
      </div>
         ...
         filtered.length === 0 ? (
          ...
          return (
            <div key={n.id} style={{ background: isSOS ? '#FBE6E2' : 'var(--surface)', border: isSOS ? '1.5px solid #C0392B' : '1px solid var(--line-soft)', borderRadius: 12, padding: '18px 20px', display: 'flex', alignItems: 'center', gap: 16, opacity: n.lida && !isSOS ? 0.6 : 1, boxShadow: isSOS ? '0 14px 30px -22px rgba(192,57,43,.6)' : 'none' }}>
              {!n.lida && <span style={{ width: 9, height: 9, borderRadius: '50%', background: '#10847D', flexShrink: 0 }} />}
