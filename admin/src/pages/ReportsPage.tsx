          ...
          {generating ? (
            <>
              ...
              <div style={{ height: 6, borderRadius: 100, background: '#E6DDC9', overflow: 'hidden' }}>
                <div style={{ width: `${Math.min(progress, 100)}%`, height: '100%', background: '#10847D', borderRadius: 100, transition: 'width 0.3s ease' }} />
              </div>
            </>
          ) : done ? (
            ...
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {/* CSV option */}
                <div onClick={() => setFormat('csv')} style={{ display: 'flex', alignItems: 'center', gap: 12, background: format === 'csv' ? '#F3ECDC' : '#fff', border: `2px solid ${format === 'csv' ? '#10847D' : '#E6DDC9'}`, borderRadius: 12, padding: 13, cursor: 'pointer' }}>
                ...
                <div onClick={() => setFormat('pdf')} style={{ display: 'flex', alignItems: 'center', gap: 12, background: format === 'pdf' ? '#F3ECDC' : '#fff', border: `2px solid ${format === 'pdf' ? '#10847D' : '#E6DDC9'}`, borderRadius: 12, padding: 13, cursor: 'pointer' }}>
                  <div style={{ width: 40, height: 40, borderRadius: 11, background: '#F7E3D6', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#A94C25" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 3H6a2 2 0 00-2 2v14a2 2 0 002 2h12a2 2 0 002-2V9z"/><polyline points="14 3 14 9 20 9"/></svg>
                  </div>
                  <div style={{ flex: 1 }}>
              ...
              <button onClick={generate} style={{ width: '100%', height: 48, border: 'none', borderRadius: 100, background: '#10847D', color: '#fff', fontWeight: 700, fontSize: 15, cursor: 'pointer', boxShadow: '0 14px 24px -14px rgba(20,168,160,.85)' }}>Gerar relatório</button>
            </>
          )}
        </div>
      </div>
